...
class User(UserMixin, db.Model):
    ...
    def verify_reset_password(token:str):
        ...
        """
        try:
            cur_id = jwt.decode(token, current_app.config['SECRET_KEY'], \
    ...
    def add_notification(self, name, data):
        """
        Add new notifications to client
        """
...
class Message(db.Model):
    """
    Represent private message model into DB
    """
    id = db.Column(db.Integer, primary_key=True)
...
class Notification(db.Model):
    """
    Represent notification model for count private messages
    """
    id = db.Column(db.Integer, primary_key=True)
...
class Task(db.Model):
    """
    Create long polling task with help Redis
    """
